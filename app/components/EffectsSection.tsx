...
interface Effect {
  id: string;
  name: string;
  icon: string; // Ahora será una clase de Font Awesome
  shortDesc: string;
  fullDesc: string;
  vimeoId: string;
  tags: string[];
...
function EffectModal({ effect, isOpen, onClose, onQuote }: EffectModalProps) {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-4xl bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] rounded-3xl overflow-hidden border border-purple-600/30 shadow-2xl shadow-purple-600/20 animate-slideUp">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-10 h-10 bg-black/80 hover:bg-purple-600 rounded-full flex items-center justify-center transition-all hover:scale-110"
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl flex items-center justify-center flex-shrink-0">
              <i className={`${effect.icon} text-3xl text-white`}></i>
            </div>
            <div>
              <h3 className="text-3xl font-bold text-white mb-1">{effect.name}</h3>
              <p className="text-gray-400 text-sm">Efecto especial profesional</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <span className="text-sm text-gray-400 font-semibold">Ideal para:</span>
            {effect.tags.map((tag, index) => (
              <span
                key={index}
                className="px-3 py-1 bg-purple-600/20 border border-purple-600/30 rounded-full text-sm text-purple-300 font-medium"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-3 rounded-full font-semibold text-white hover:scale-105 transition-all shadow-lg shadow-pink-600/30"
            >
              Cotizar Este Efecto
            </button>
            <button
              onClick={onClose}
              className="flex-1 border-2 border-purple-600 px-6 py-3 rounded-full font-semibold text-white hover:bg-purple-600/10 transition-all"
            >
              Ver Más Efectos
            </button>
          </div>
...
export default function EffectsSection({ onOpenQuoteModal }: EffectsSectionProps) {
  ...
  return (
    <>
      <section id="efectos" className="section py-20 px-8 bg-black relative overflow-hidden">
        {/* Fondo decorativo */}
        <div className="absolute inset-0 z-0">
          <div className="absolute top-20 left-[10%] w-64 h-64 bg-purple-600/10 rounded-full blur-[100px] animate-pulse"></div>
          <div className="absolute bottom-32 right-[15%] w-80 h-80 bg-pink-600/10 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '2s' }}></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="text-center mb-16">
            <div className="inline-block mb-4">
              <span className="px-4 py-2 bg-purple-600/20 border border-purple-600/30 rounded-full text-sm text-purple-300 font-semibold">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {effects.map((effect, index) => (
              <div
                ...
                className="group relative bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] rounded-2xl overflow-hidden border border-purple-600/20 hover:border-purple-500 transition-all duration-300 cursor-pointer hover:-translate-y-2 hover:shadow-2xl hover:shadow-purple-600/30"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                {/* Thumbnail Background */}
                <div className="relative h-48 bg-gradient-to-br from-purple-900/30 to-pink-900/30 overflow-hidden">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <i className={`${effect.icon} text-7xl opacity-30 group-hover:scale-110 transition-transform duration-300 text-white`}></i>
                  </div>
                  {/* Play overlay */}
                ...
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-12 h-12 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl flex items-center justify-center flex-shrink-0">
                      <i className={`${effect.icon} text-2xl text-white`}></i>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {effect.tags.slice(0, 2).map((tag, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-1 bg-purple-600/10 rounded-full text-xs text-purple-300"
                      >
                        {tag}
                      </span>
                    ))}
                    {effect.tags.length > 2 && (
                      <span className="px-2 py-1 bg-purple-600/10 rounded-full text-xs text-purple-300">
                        +{effect.tags.length - 2}
                      </span>
                    )}
                  </div>
                  ...
                  {/* CTA */}
                  <button className="w-full text-pink-500 font-semibold hover:text-pink-400 flex items-center justify-center gap-2 group-hover:gap-3 transition-all">
                    Ver Demostración
