...
const INSTAGRAM_POSTS: InstagramPost[] = [
  {
    id: '1',
    image: '/img/instagram/publi1.jpg',
  ...
  },
  {
    ...
    caption: 'Pirotecnia para bodas inolvidables ',
  ...
  },
  {
    ...
    caption: 'Bodas mágicas con Niebla Baja',
  ...
  },
  {
    ...
    caption: 'Bodas de ensueño con fuentes frías ',
  ...
  },
  {
    ...
    caption: 'Resumen de efectos especiales para eventos',
  ...
  },
  {
    ...
    caption: 'Escribir contigo la eternidad',
...
export default function InstagramGrid() {
  ...
  return (
    <>
      {/* Grid 3x3 - En móvil muestra 6, en desktop muestra 9 */}
      <div className="grid grid-cols-3 gap-2 md:gap-3 lg:gap-4 max-w-3xl mx-auto">
        {INSTAGRAM_POSTS.map((post, index) => (
          <a
            ...
            className={`relative aspect-square group overflow-hidden rounded-lg bg-gray-900 ${
              index >= 6 && !showAllMobile ? 'hidden md:block' : ''
            }`}
          ...
          {post.type === 'video' && (
            <div className="absolute top-2 right-2 md:top-3 md:right-3 z-10">
              <div className="w-6 h-6 md:w-8 md:h-8 bg-black/70 rounded-full flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex items-center justify-center p-3 md:p-4 text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
            <p className="text-white font-semibold text-xs md:text-sm line-clamp-3">
              {post.caption}
            </p>
          </div>

          {/* Icono de Instagram en hover - SOLO CSS */}
          <div className="absolute bottom-2 right-2 md:bottom-3 md:right-3 opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300">
    ...
    {/* Botón "Ver más" solo en móvil */}
    {!showAllMobile && (
      <div className="flex justify-center mt-6 md:hidden">
        <button
          onClick={() => setShowAllMobile(true)}
          className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-full font-semibold text-sm text-white hover:scale-105 transition-all shadow-lg shadow-pink-600/30 flex items-center gap-2"
        >
          <span>Ver los 3 restantes</span>
